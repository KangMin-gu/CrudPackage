		...
		$.ajax({
			...
			success: function(result){         		    
				...
				for(var i=0; i<result.length; i++){
					$('#calendar').fullCalendar('renderEvent', result[i]);     	
				}
			},
			error:function(){
				alert('fail');	
	...
	$('#calendar').fullCalendar({
		...
		dayClick: function(e) {//캘린더 day 클릭 e._d => 클릭한 날짜
			...
			openNewWindow("캘린더추가","/sales/cal/post?startdate="+startDate,"",600,700);//팝업호출
		...
		eventDrop: function(event, delta, revertFunc) {
			...
			if (!confirm("일정을 변경하시겠습니까?")) {
				...
				$.ajax({
					url: '/sales/cal/post/'+id+'?startdate='+startdate+'&enddate='+enddate,
					type: 'get',
					dataType: 'json',
					//data: test,
					processData: false, 
